...
> Phase 2.1 (`cc1` Emscripten port + end-to-end) ✅ landed 2026-05-15 —
> `cc1.wasm` (12 MB, 3.3 MB brotli) + `cc1.mjs` ES module compiles
> `spike/hello_toolbox.c` against Retro68's full SDK sysroot
> (mounted into MEMFS) and emits m68k assembly **byte-for-byte
> identical to the native cross-cc1**. See [`spike/wasm-cc1/`](./spike/wasm-cc1/).
...
| ✅ Phase 2.1 — Emscripten port of `m68k-apple-macos-cc1` + end-to-end test. **12 MB raw wasm, 3.3 MB brotli**. Compiles `spike/hello_toolbox.c` against full Retro68 sysroot in MEMFS → m68k assembly **byte-identical to native** (`diff` exit 0). 8-round iteration trail (CONFIG_SITE, -Os -g0 vs finalize OOM, custom relink to inject wasm flags, NODEFS sysroot mount, etc.) in [`LEARNINGS.md`](./LEARNINGS.md). See PR #14. | 🟡 Phase 2.3b — Elf2Mac wasm port. Native build clean; wasm cross blocked on libelf (Emscripten port not available). Building elfutils for wasm as the next step. Scaffolding in [`spike/wasm-elf2mac/`](./spike/wasm-elf2mac/). | ❌ **GCC's full bootstrap (stage1/2/3 self-host).** `--disable-bootstrap`. Stage 2 builds with the host's native gcc only. Self-hosting `cc1` inside WASM is impossible without the runtime we're building. |
| ✅ Phase 2.2 — Emscripten port of `as` (binutils assembler) + end-to-end test. **764 KB raw / 270 KB brotli**. Assembles cc1's `.s` output → m68k ELF32 object file, **byte-identical to native** (`diff` exit 0). One-shot success: Phase 2.1 lessons transferred without iteration. See [`spike/wasm-binutils/`](./spike/wasm-binutils/). | ⬜ Phase 2.4 — Bundle-size optimisation pass. Already at ~3.9 MB brotli for cc1 + as + ld together (Phase 2.1 included `-Os -g0` opportunistically). Remaining lever: strip unused m68k subtargets, LTO across libbackend.a, treeshake libcpp. | ❌ **Driver / `collect2` / link-stage runner.** Emscripten has no `fork`/`exec`. We bypass GCC's driver entirely and call `cc1` directly from JS with cooked argv. Same approach for `as` and `ld`. |
| ✅ Phase 2.3a — Emscripten port of `ld` (binutils linker). **1.0 MB raw / 304 KB brotli**. Shipped alongside `as` in the Phase 2.2 build. Smoke passes (`--version`, `--help`); supports `elf32-m68k` target, `m68kelf` emulation. End-to-end link (ld → ELF executable) gated on Phase 2.3b Elf2Mac to complete the C → MacBinary pipeline. | ⬜ Phase 2.5 — npm package mirroring [`wasm-rez`](https://github.com/khawkins98/wasm-rez)'s API. Wire into classic-vibe-mac's playground. Sysroot bundling strategy (preload-file vs fetched tarball) decided here. | ❌ **PowerPC / CFM / Mac OS 8 / SheepShaver.** Long-term aspiration, not Phase 2. We target 68040 Quadra-650 under BasiliskII, mirroring classic-vibe-mac's deploy env. |
| ✅ Phase 2 research baseline. Closest precedent (Emception) reverse-engineered, canadian-cross recipe + Emscripten link flags + memory-snapshot reset trick all captured in [`LEARNINGS.md`](./LEARNINGS.md). | | ❌ **In-browser compilation of the Retro68 SDK headers themselves.** Headers are pre-built; only user code goes through the wasm `cc1`. |
| ✅ Cross-repo integration plumbing reused from Phase 1: HFS patcher, MacBinary inspector, Playwright boot tester, SHA + info.txt diagnostics. All compiler-agnostic — they will catch Phase 2 regressions the same way they caught Phase 1's. | | |
