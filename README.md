...
| ✅ Phase 2.1 — Emscripten port of `m68k-apple-macos-cc1` + end-to-end test. **12 MB raw wasm, 3.3 MB brotli**. Compiles `spike/hello_toolbox.c` against full Retro68 sysroot in MEMFS → m68k assembly **byte-identical to native** (`diff` exit 0). 8-round iteration trail (CONFIG_SITE, -Os -g0 vs finalize OOM, custom relink to inject wasm flags, NODEFS sysroot mount, etc.) in [`LEARNINGS.md`](./LEARNINGS.md). See PR #14. | 🚧 Phase 2.2 — Emscripten port of `as` (binutils assembler). Same Canadian-cross shape; the 8 landmines from 2.1 transfer. Smaller binary than cc1. | ❌ **GCC's full bootstrap (stage1/2/3 self-host).** `--disable-bootstrap`. Stage 2 builds with the host's native gcc only. Self-hosting `cc1` inside WASM is impossible without the runtime we're building. |
...
|  | ⬜ Phase 2.5 — npm package mirroring [`wasm-rez`](https://github.com/khawkins98/wasm-rez)'s API. Wire into classic-vibe-mac's playground. Sysroot bundling strategy (preload-file vs fetched tarball) decided here. | ❌ **In-browser compilation of the Retro68 SDK headers themselves.** Headers are pre-built; only user code goes through the wasm `cc1`. |
